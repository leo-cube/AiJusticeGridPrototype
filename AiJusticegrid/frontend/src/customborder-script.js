...
const API_ENDPOINT = `${API_BASE_URL}/api/customborder`;
...
async function startInvestigation() {
    ...
    try {
        ...
        addMessage('agent', 'Sorry, I encountered an error starting the inspection. Please try again.');
    } finally {
        showLoading(false);
    }
}
    ...
    try {
        ...
        if (data.success) {
            ...
            if (data.data.analysis_complete) {
                ...
                addMessage('agent', '🎉 **Inspection Complete!** You can now download the comprehensive PDF report.');
            }
        } else {
...
function addMessage(sender, content) {
    ...
    avatar.innerHTML = sender === 'agent' ? '<i class="fas fa-passport"></i>' : '<i class="fas fa-user"></i>';
...
function resetConversation() {
    if (confirm('Are you sure you want to reset the inspection? All progress will be lost.')) {
        sessionId = null;
        currentStep = 0;
        ...
        chatMessages.innerHTML = `
            <div class="welcome-message">
                ...
                <div class="welcome-content">
                    <h2>Welcome to Customs Border Protection Assistant</h2>
                    <p>I'm your specialized AI assistant for customs border protection operations. I'll guide you through a comprehensive border inspection analysis process.</p>
...
async function downloadPDF() {
    if (!sessionId || !isAnalysisComplete) {
        alert('Please complete the inspection before downloading the report.');
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            a.href = url;
            a.download = `customs_border_report_${sessionId}.pdf`;
